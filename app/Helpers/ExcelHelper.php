...
class ExcelHelper
{
     ...
     * @return bool
     */
    public static function exportCSVDataByFile($list = [], $header = [], $filename = '', $type = 'output', $dir = '/')
    {
...
//                暂时不开启自动清理文件功能
                $lock_file = $dir . date('Y-m-d') .".excel_lock";
                if (!file_exists($lock_file)) {
                    ...
                    foreach ($files as $file) {
                        ...
                        if (!in_array($ext_name, ['excel_lock', 'xlsx', 'xls', 'csv'])) {
                            return;
                        }
                        $excel_file_name = $dir.$file;
                        if (filemtime($excel_file_name) < $time) {
                ...
                $name = $dir . $filename . ".csv";
                $name = iconv("utf-8", "gb2312", $name); // 重命名表
                $handle = fopen($name, 'a' );

                if (filesize($name) == 0) {
                    foreach($header as $li)
                    {
                        $headerArr[] = iconv('utf-8', 'gbk', $li->name);
                    }
                ...
                foreach($list as $row)
                {
                    ...
                    foreach($header as $key => $value)
                    {
                        $data[] = '"'.$row[$value->field] ? iconv('utf-8', 'gbk', $row[$value->field]) . '"\t' : '';
                    }
                    fputcsv($handle, $data);
                    unset($data);
                }
