...
use App\Common\Enums\StorageEnum;
use App\Common\Enums\UploadEnum;
use App\Helpers\Upload\UploadDrive;
...
class UploadHelper
{
     ...
     * @throws \Exception
     */
    public function __construct($files, $type, $superaddition = false)
    {
        // 设置文件类型
        $this->type = $type;
        // 获取配置信息
        ...
        $drive = $this->drive;
        // 文件驱动
        $this->uploadDrive = app(UploadDrive::class);
        // 文件驱动对应类
        $this->uploadDrive = $this->uploadDrive->$drive([
     ...
     * @return Config
     */
    public function prepare($files, $config = array())
    {
        //对应文件配置信息
        $this->config = !empty($config) ? $config : config("filesystems.uploadConfig.".$this->type);

        !empty(request()->drive) && $this->drive = request()->drive;

        if (empty($files)) {
            throw new \Exception('找不到上传文件');
        }

        if (isset($files[$this->uploadFileName])) {
            // 文件操作实例
            $this->file = $file = $files[$this->uploadFileName];
        } else {
            $this->file = $files;
        }
    ...
    public function verifyFile()
    {
        ...
        if (!$file->isValid()) {
            return '上传失败，请检查文件'.$file->getErrorMessage();
        }

        $this->baseInfo['extension'] = $file->getClientOriginalExtension();
        $this->baseInfo['size'] = $file->getClientSize();

        empty($this->baseInfo['name']) && $this->baseInfo['name'] = $file->getBaseName();
        $this->baseInfo['url'] = $this->paths['relativePath'] . $this->baseInfo['name'] . '.' . $file->getClientOriginalExtension();
    ...
    public function verifyUrl($url)
    {
        ...
        if (strpos($imgUrl, "http") !== 0) {
            throw new \Exception('不是一个http地址');
        }
    ...
    protected function verify()
    {
        ...
        // 存储本地进行安全校验
        if ($this->drive == StorageEnum::DRIVE_LOCAL) {
            if ($this->type == UploadEnum::UPLOAD_TYPE_FILES && in_array($this->baseInfo['extension'],
                    $this->config['blacklist'])) {
                throw new \Exception('上传的文件类型不允许');
            }
        }
    }
    ...
    public function save($data = false)
    {
        ...
        if ($this->filesystem->has($this->baseInfo['url'])) {
            $this->baseInfo['name'] = $this->baseInfo['name'] . '_' . Str::random(16);
            $this->baseInfo['url'] = $this->paths['relativePath'] . $this->baseInfo['name'] . '.' . $this->baseInfo['extension'];
        }

        // 判断是否直接写入
        if (false === $data) {
            $this->baseInfo['realPath'] = $this->config['rootPath'] .'/'. $this->baseInfo['url'];
...
//            // 图片压缩
    ...
    {
//        if (config('filesystems.sys_image_watermark_status') != true) {
//            return true;
//        }

        Image::make($this->baseInfo['realPath'])
            ->insert('watermark.jpg', 'bottom-right', 15, 10)
            ->save($this->baseInfo['realPath']);
    ...
    {
//        if (config('filesystems.compress') != true) {
//            return true;
//        }

        // 原图路径
        $absolutePath = $this->baseInfo['realPath'];

        $compressibility = config('filesystems.uploadConfig.images.compressibility');
        $tmpMinSize = 0;
    ...
    {
        $thumbs = json_decode(request()->post('thumb'));
        ...
        // 缩略图路径
        $path = $this->config['rootPath']  .'/'. $this->paths['thumbRelativePath'];
        ...
        foreach ($thumbs as $value) {
            ...
            Image::make($this->baseInfo['realPath'])->resize($value->width, $value->height)->save($thumbFullPath);
        }
    ...
    protected function initPaths()
    {
        ...
        $config = $this->config;
        // 保留原名称
        $this->config['originalName'] == false && $this->baseInfo['name'] = $config['prefix'] . Str::random(16);

        // 文件路径
        $filePath = $config['path'] . date($config['subName'], time()) . "/";

        // 缩略图
        $thumbPath = 'thumbnail/' . date($config['subName'], time()) . "/";

        empty($config['guid']) && $config['guid'] =  Str::random(16);
    ...
    public function getBaseInfo()
    {
        ...
        $this->baseInfo['formatter_size'] = $this->baseInfo['size'];
    ...
    public static function formattingFileType($specific_type, $extension, $upload_type)
    {
        if (preg_match("/^image/", $specific_type) && $extension != 'psd') {
            return UploadEnum::UPLOAD_TYPE_IMAGES;
        }
